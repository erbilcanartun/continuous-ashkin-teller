import numpy as np
import mpmath as mp
from mpmath import sin, cos, pi, exp, matrix, conj
...
def display_coefficient_matrix(flow_history, step, decimals=4):
    ...
    # Set up mpmath display precision
    mp.dps = decimals + 4  # Add some extra precision for internal calculations

    # Print header with step information
    #print(f"Coefficient Matrix at RG Step {step}\n")
    ...
    for i, row_label in enumerate(row_labels):
        ...
        for j in range(matrix.shape[1]):
            # Convert mpmath number to string with desired precision
...
def display_coefficient_matrix_colormap(matrix, decimals=4, component='real',
    ...
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            ...
            # Handle different types of values (mpmath, complex, float)
            if hasattr(val, 'real'):  # Complex or mpmath number
                if component == 'real':
                    # For mpmath numbers, convert to string then float
                    if hasattr(val.real, '_mpf_'):  # mpmath number
                        display_matrix[i, j] = float(mp.nstr(val.real, n=decimals+2))
                    else:  # regular complex
                        display_matrix[i, j] = float(val.real)
                elif component == 'imag':
                    if hasattr(val.imag, '_mpf_'):  # mpmath number
                        display_matrix[i, j] = float(mp.nstr(val.imag, n=decimals+2))
                    else:  # regular complex
                        display_matrix[i, j] = float(val.imag)
                else:  # 'abs'
                    if hasattr(abs(val), '_mpf_'):  # mpmath number
                        display_matrix[i, j] = float(mp.nstr(abs(val), n=decimals+2))
                    else:  # regular complex
                        display_matrix[i, j] = float(abs(val))
        ...
        for m in range(start_idx, end_idx):
            ...
            if hasattr(val, 'real'):  # Complex or mpmath number
                if component == 'real':
                    # For mpmath numbers, convert to string then float
                    if hasattr(val.real, '_mpf_'):  # mpmath number
                        val_to_display = float(mp.nstr(val.real, n=decimals+2))
                    else:  # regular complex
                        val_to_display = float(val.real)
                elif component == 'imag':
                    if hasattr(val.imag, '_mpf_'):  # mpmath number
                        val_to_display = float(mp.nstr(val.imag, n=decimals+2))
                    else:  # regular complex
                        val_to_display = float(val.imag)
                else:  # 'abs'
                    if hasattr(abs(val), '_mpf_'):  # mpmath number
                        val_to_display = float(mp.nstr(abs(val), n=decimals+2))
                    else:  # regular complex
                        val_to_display = float(abs(val))
            else:  # Already a float or similar
...
def plot_coefficient_evolution(flow_history, J1, J2, M):
    ...
    for n, m in coeff_indices:
        ...
        for step in range(n_steps):
            values[step] = float(flow_history[step, n_idx, m_idx].real)
